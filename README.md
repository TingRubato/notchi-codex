...
A macOS notch companion that reacts to Claude Code, Codex, and gemini-cli activity in real-time.

https://github.com/user-attachments/assets/e417bd40-cae8-47c0-998a-905166cf3513

## What it does

- Reacts to Claude Code, Codex, and gemini-cli events in real-time (thinking, working, errors, completions)
...
- Supports multiple concurrent CLI sessions with individual sprites
...
- macOS 15.0+ (Sequoia)
- MacBook with notch
- One or more supported CLIs installed (Claude Code, Codex, gemini-cli)
...
3. Launch Notchi — it auto-installs hooks on first launch
4. A macOS keychain popup may appear asking to access Claude Code's cached OAuth token (used for Claude API usage stats). Click **Always Allow** so it won't prompt again on future launches
...
6. Start using your supported CLI and watch Notchi react

## How it works

```
CLI (Claude/Codex/gemini-cli) --> Hooks (shell scripts) --> Unix Socket --> Event Parser --> State Machine --> Animated Sprites
```

Notchi registers shell script hooks on launch. When a supported CLI emits events (tool use, thinking, prompts, session start/end), the hook script sends JSON payloads to a Unix socket. The app parses these events, runs them through a state machine that maps to sprite animations (idle, working, sleeping, compacting, waiting), and uses the Anthropic API to analyze user prompt sentiment for emotional reactions.

Each session gets its own sprite on the grass island. Clicking expands the notch panel to show a live activity feed, session info, and API usage stats.
